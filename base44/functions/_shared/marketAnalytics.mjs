// Aggregate market analytics over AircraftListing records.
//
// Extracted verbatim from computeMarketAnalytics/entry.ts so that the ABOS app
// dashboard and the White-Label tenant surface compute market intelligence from
// ONE implementation. Two copies of this arithmetic would drift, and a tenant
// being told a different median price than ABOS's own dashboard shows is the
// kind of discrepancy nobody notices until a customer does.
//
// Pure and synchronous on purpose: no Deno imports, no I/O, no clock of its
// own. That makes it directly executable under `node --test`, unlike the Deno
// entry points that call it.

const MODEL_KEY = (l) => `${(l.make || '').trim()} ${(l.model || '').trim()}`.trim() || 'Unknown';

/**
 * @param {Array<object>} listings AircraftListing records
 * @param {Date} now Reference time — injected rather than read from the clock
 *   so month bucketing and days-on-market are reproducible in tests.
 * @returns {object} The analytics payload (shape unchanged from the original).
 */
export function computeMarketAnalytics(listings, now = new Date()) {
  // ── Summary ──
