...
Deno.serve(async (req) => {
  try {
    ...
    if (!nNumber || typeof nNumber !== 'string') {
      return Response.json({ source: 'engine_database', data: null, fetched_at: new Date().toISOString(), status: 'error', error: 'nNumber is required' }, { status: 400 });
    }
    ...
    try {
      ...
      if (regResult?.data?.engine_type) engineType = regResult.data.engine_type;
    } catch (_) {
      // Continue without
    }

    // TODO: Replace with Supabase query when credentials are provided
    // This function will query the aircraft/engine reference database in Supabase
