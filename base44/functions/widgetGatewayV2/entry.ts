...
/**
 * Widget Gateway V2 — validates embed_token and routes intelligence actions
...
Deno.serve(async (req) => {
  try {
    ...
    const expectedSecret = Deno.env.get('ABOS_GATEWAY_SHARED_SECRET');
    if (!expectedSecret) {
      console.error('widgetGatewayV2: ABOS_GATEWAY_SHARED_SECRET not configured in Base44 env');
      return Response.json({ error: 'gateway_secret_not_configured' }, { status: 503 });
    }
    const providedSecret = req.headers.get('x-abos-gateway-secret') || '';
    ...
    console.error('widgetGatewayV2 error:', error);
...
async function lookupRegistry(base44, rawReg) {
  ...
  results.forEach((r, i) => {
    ...
    else { failed.push(name); console.warn(`widgetGatewayV2 lookup ${name} failed:`, r.reason?.message || r.reason); }
  });
