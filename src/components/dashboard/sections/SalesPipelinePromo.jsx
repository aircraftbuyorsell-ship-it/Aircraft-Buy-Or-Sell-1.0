...
{ icon: FileText, label: "AI Document Review", desc: "Logbooks, damage history and AD/SB compliance", color: "#D4A017" },
  ...
  return (
    <SectionShell
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-6">
        {STEPS.map((s, i) =>
        <div key={s.label} className="relative glass-card p-4"
        style={{ borderLeft: `2px solid ${s.color}` }}>
            <span className="absolute top-3 right-3 text-[10px] font-black" style={{ color: `${s.color}90` }}>
              {i + 1}
            </span>
          ...
          style={{ background: `${s.color}15` }}>
            ...
            <p className="text-[12px] font-bold text-foreground mb-1">{s.label}</p>
            <p className="text-[10.5px] leading-relaxed text-muted-foreground">{s.desc}</p>
          </div>
        )}
      </div>
      ...
      <div className="flex justify-center">
        ...
        className="inline-flex items-center gap-2 px-6 py-3 rounded-[10px] text-[13px] font-bold bg-gold-official text-white hover:opacity-90 transition-opacity">
