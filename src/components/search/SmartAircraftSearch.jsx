...
const HINTS = [
  "TRY N12345 — FULL AIRCRAFT HISTORY",
  "SEARCH BY SERIAL NUMBER…",
  "FIND AN OWNER OR DEALER…",
  "CHECK ENGINE & TBO DATA…",
  "ESTIMATE OPERATING COSTS…",
  "TRACK LIVE AIRCRAFT ACTIVITY…",
];
...
export default function SmartAircraftSearch({ variant = "hero", value, onChange, onSubmit, onNavigate, loading = false }) {
  ...
  const hint = useTypewriterPlaceholder(HINTS);
  const typedHint = query ? "" : `${hint}▌`;
  ...
  return (
    <div ref={rootRef} className={`relative w-full ${compact ? "max-w-md" : "max-w-xl"}`}>
      <form onSubmit={submit} className={`flex w-full items-stretch overflow-visible rounded-xl border border-primary/30 bg-background/100 shadow-xl ${compact ? "min-h-11" : "min-h-14"}`}>
        ...
        <input ref={inputRef} value={query} onChange={(event) => update(event.target.value)} onFocus={() => setOpen(Boolean(query.trim()))} placeholder={typedHint} aria-label="Search ABOS aircraft intelligence" className="min-w-0 flex-1 border-0 bg-transparent px-1 text-foreground outline-none focus-visible:ring-0 opacity-100 text-sm placeholder:font-mono placeholder:tracking-wide" />
        <button type="button" onClick={() => setOpen((current) => !current)} aria-label="Show search tools" className="hidden sm:flex min-h-11 items-center border-l border-border px-2 text-muted-foreground hover:text-foreground"><ChevronDown className="h-4 w-4" /></button>
