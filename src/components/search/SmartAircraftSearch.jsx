  ...
  return (
    <div ref={rootRef} className={`relative w-full ${compact ? "max-w-md" : "max-w-xl"}`}>
      <form onSubmit={submit} className={`flex w-full items-stretch overflow-visible rounded-xl border border-primary/30 bg-background/100 shadow-xl ${compact ? "min-h-11" : "min-h-14"}`}>
        ...
        <input ref={inputRef} value={query} onChange={(event) => update(event.target.value)} onFocus={() => setOpen(Boolean(query.trim()))} placeholder="N-number, S/N, owner, activity, dealer, engine, costs…" aria-label="Search ABOS aircraft intelligence" className="min-w-0 flex-1 border-0 bg-transparent px-1 text-foreground outline-none focus-visible:ring-0 opacity-100 text-sm" />
