...
export default function StElmoChat() {
      ...
      try {
        ...
        if (!cancelled && task) {
          ...
          if (task.status === "completed" && task.result) {
            ...
            setBusy(false); setBackgroundTask(null); setTaskId(null); return;
          }
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      if (task?.id) { try { await updateStElmoTask(task.id, { status: "failed", phase: "failed", error: message }); } catch {} }
    ...
    <div className="fixed bottom-4 right-4 z-[80] font-sans">
      {open && <div className={`mb-3 overflow-hidden rounded-2xl border border-white/10 bg-[#0b1220]/95 shadow-2xl shadow-black/40 backdrop-blur-xl ${expanded ? "h-[min(78vh,720px)] w-[min(92vw,760px)]" : "h-[min(68vh,620px)] w-[min(92vw,430px)]"}`}>
        ...
        <div className="flex h-[calc(100%-116px)] flex-col overflow-y-auto px-4 py-4 bg-gray-50">
          ...
          <div className="space-y-3">{messages.map((message, index) => <div key={index} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}><div className={`max-w-[88%] rounded-2xl px-3.5 py-2.5 text-xs leading-5 ${message.role === "user" ? "rounded-br-sm bg-[#0B2D5B] text-white" : "rounded-bl-sm border border-white/10 bg-white/[0.055] text-white/85"}`}>{message.role === "assistant" ? <ReactMarkdown>{message.content}</ReactMarkdown> : <p>{message.content}</p>}{message.meta && <p className="mt-1 border-t border-white/10 pt-1 text-[8px] uppercase tracking-wider text-white/25">{message.meta.model} · {message.meta.provider}</p>}</div></div>)}{busy && <div className="flex items-center gap-2 px-2 text-[10px] text-[#E8A83A]"><Loader2 className="h-3.5 w-3.5 animate-spin" /> St. Elmo is reasoning…</div>}<div ref={endRef} /></div>
        </div>
      ...
      {!open && <button onClick={() => setOpen(true)} className="group flex items-center gap-2 rounded-full border border-[#E8A83A]/35 bg-[#0b1220]/95 px-3 py-2.5 shadow-xl shadow-black/30 backdrop-blur-xl transition hover:border-[#E8A83A]/70 hover:scale-[1.02]" aria-label="Open St. Elmo M_1.0"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#E8A83A] text-[#0B1220]"><BrainCircuit className="h-4 w-4" /></span><span className="text-left"><span className="block text-[11px] font-black text-white">St. Elmo</span><span className="block text-[8px] uppercase tracking-wider text-white/35">M_1.0 · online</span></span></button>}
    </div>
  );
}
