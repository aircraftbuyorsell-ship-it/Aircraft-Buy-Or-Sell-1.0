...
import { Calculator, TrendingUp, DollarSign, Percent, Calendar, Plane, Info, ShieldCheck, Gauge, FileText, ArrowRight } from "lucide-react";
...
export default function LeasingCalculator() {
  ...
  // Inputs (pre-filled from URL params — e.g. coming from registry lookup)
  const [aircraftPrice, setAircraftPrice] = useState(() => {
    const p = parseFloat(readParam("asking_price"));
    return p > 0 ? p : 850000;
  });
  ...
  const calc = useMemo(() => {
    ...
    // Lease payment: finance (depreciation portion) + interest on avg balance
    const depreciationFinanced = financedAmount - residualValue;
    const depreciationMonthly = depreciationFinanced / termMonths;
    // Interest on average outstanding balance (simplified money-factor style)
  ...
  return (
    <div className="min-h-screen pb-16" style={{ background: "transparent", color: "#fff", fontFamily: "Inter, -apple-system, sans-serif" }}>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-5 gap-5">
        {/* ── Inputs ── */}
        ...
        <div className="lg:col-span-3 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Link
              to={`/valuation?make=${encodeURIComponent(make)}&model=${encodeURIComponent(model)}&year=${year}&asking_price=${aircraftPrice}`}
