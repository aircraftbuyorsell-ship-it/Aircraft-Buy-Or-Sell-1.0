...
import { Plane, RefreshCw, AlertCircle, Navigation, Gauge } from "lucide-react";
...
function makeIcon(onGround, hasATI) {
  const color = hasATI ? "#D4A017" : onGround ? "#AAA49C" : "#185FA5";
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <p className="text-[#E8A83A]/70 text-[9px] uppercase tracking-[0.25em] font-bold mb-1">
              OpenSky Network · ADS-B · ~15 min delay
            </p>
        ...
        {aircraft.length === 0 && !loading && !error && (
          <div className="absolute inset-0 flex flex-col items-center justify-center z-[999] pointer-events-none">
            <div className="bg-white/90 backdrop-blur-sm border border-black/[0.08] rounded-2xl px-6 py-5 text-center shadow-lg pointer-events-auto">
              ...
              <p className="text-[11px] text-[#6B6560]">Data is ~15 minutes delayed from OpenSky</p>
            </div>
          </div>
        )}
        ...
        >
          ...
          {aircraft.map((ac) => (
            <Marker
              ...
              icon={makeIcon(ac.on_ground, !!ac.listing)}
            >
              <Popup maxWidth={240}>
