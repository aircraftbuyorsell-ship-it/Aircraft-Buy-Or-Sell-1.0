...
import { Plane, Check, ArrowRight, Mail, MapPin, Sparkles, ArrowLeft } from "lucide-react";
...
export default function SkylarkConfigurator() {
  ...
  return <main className="min-h-screen bg-[#f7f4ef] text-[#111827]">
    <header className="border-b border-black/10 bg-white/80 backdrop-blur sticky top-0 z-20"><div className="max-w-6xl mx-auto px-5 py-4 flex items-center justify-between"><div className="flex items-center gap-3"><button onClick={() => navigate(-1)} aria-label="Go back" className="flex items-center justify-center min-h-11 min-w-11 rounded-xl border border-black/10 bg-white/5"><ArrowLeft className="w-5 h-5" /></button><div className="w-9 h-9 rounded-xl bg-[#d6ad45] flex items-center justify-center"><Plane className="w-5 h-5" /></div><div><div className="font-black tracking-tight">AIRCRAFT BUY OR SELL</div><div className="text-[10px] uppercase tracking-[.18em] text-black/45">LANDA Aircraft · Skylark</div></div></div><BottomSheetSelect value={language} onChange={v => setLanguage(v)} options={[{ value: "en", label: "English" }, { value: "cs", label: "Čeština" }]} label="Language" className="text-xs font-bold" /></div></header>
    <div className="max-w-6xl mx-auto px-5 py-10 md:py-16 grid lg:grid-cols-[1.2fr_.8fr] gap-10">
      ...
      <form onSubmit={submit} className="bg-white rounded-3xl border border-black/10 shadow-lg p-6 md:p-8 space-y-7">
        ...
        <div><p className="text-xs uppercase tracking-[.16em] font-bold text-black/40">04 · {cs ? "Barva" : "Paint"}</p><div className="mt-3"><BottomSheetSelect value={paint} onChange={v => setPaint(v)} options={[{ value: "Standard", label: "Standard" }, { value: "Custom", label: "Custom" }]} label={cs ? "Barva" : "Paint"} /></div></div>
