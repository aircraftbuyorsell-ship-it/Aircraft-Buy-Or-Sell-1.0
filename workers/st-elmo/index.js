...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "GET" && url.pathname === "/") {
      return json({ service: MODEL_ID, version: VERSION, status: "online", backend: "nvidia" });
    }

    if (request.method === "GET" && url.pathname === "/health") {
      return json({ ok: true, model: MODEL_ID, version: VERSION });
    }
    ...
    if (!env.NVIDIA_API_KEY) {
      return json({ error: "NVIDIA_API_KEY is not configured" }, 503);
    }
    ...
    if (!response.ok) {
      ...
      return json({ error: "NVIDIA backend request failed", status: response.status, detail: detail.slice(0, 2000) }, 502);
    }
